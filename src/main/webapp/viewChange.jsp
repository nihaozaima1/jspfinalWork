...
<body>
<div class="main">
  <% view temp = (view) session.getAttribute("changedView");
  %>
  <h2>编号:<%=temp.getId()%></h2>
  <form action="ViewChangeServlet" method="get" class="form">
    菜名: <input type="text" name="viewname" value=<%=temp.getName()%>><br>    <!--输入的用户名保存到username变量，传回后端-->
    特点: <input type="text" name="characteristic" value=<%=temp.getCharacteristic()%>><br>
